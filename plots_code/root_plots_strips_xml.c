...
c1->Divide(2,6);
TFile *file0 = TFile::Open("DQM_V0001_CTPPS_R000369585_strips_xml.root");
TH2F* hh = nullptr;
TH1F* hh1 = nullptr;
...
for(int i =0;i<2;i++){
    ...
    for(int j=0;j<4;j++){
        ...
        sprintf(result, "strips_%s_210_%s_xml.pdf", a[i], b[j]);
        c1->SaveAs(result);
    }
    ...
    for(int j=0;j<2;j++){
        ...
        sprintf(result, "strips_%s_220_%s_xml.pdf", a[i], b[j]);
        c1->SaveAs(result);
    }
